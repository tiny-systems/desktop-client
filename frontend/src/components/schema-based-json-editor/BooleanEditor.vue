<template>
  <div :class="className">
    ...
    <div v-if="value !== undefined" :class="['flex justify-between mx-1', deleteHover ? 'bg-red-100 dark:bg-red-900/50 border-0 rounded' : (hover ? 'bg-indigo-100 dark:bg-indigo-800 border-0 rounded' : '')]">
      ...
      <button type="button" class="w-4 text-indigo-500 cursor-pointer" @mouseover="hover = true" @mouseleave="hover = false" v-if="allowLookup && value !== undefined" @click="$emit('lookup', getAllValue(), schema, onChangeExpression)" :title="expression ? 'Edit expression' : 'Apply expression'">
...
export default {
  ...
  methods: {
    ...
    toggleOptional() {
      ...
      //@ts-ignore
      this.value = common.toggleOptional(this.value, this.schema, fallbackValue) as boolean | undefined
      this.emitValue()
    },
    emitValue() {
      //@ts-ignore
